...
async function fetchDurationSeconds(url: string): Promise<number | null> {
  ...
  const timeout = setTimeout(() => controller.abort(), env.JOB_REQUEST_TIMEOUT_MS);
  try {
...
export async function runTrackDurations(): Promise<void> {
  ...
  for (;;) {
    const remaining = env.JOB_MAX_TRACKS === undefined ? env.JOB_BATCH_SIZE : env.JOB_MAX_TRACKS - scanned;
    ...
    const batch: Candidate[] = rows.flatMap((row) =>
      row.previewUrl === null ? [] : [{ id: row.id, previewUrl: row.previewUrl }],
    );
    ...
    console.log(`[track-durations] progress: scanned=${scanned} updated=${updated} failed=${failed}`);
  }

  console.log(`[track-durations] done: scanned=${scanned} updated=${updated} failed=${failed}`);
}
